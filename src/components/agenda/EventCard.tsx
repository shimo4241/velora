...
import { toggleEventInterest, subscribeToEventAttendees } from "@/lib/firestore";
...
export const EventCard: React.FC<EventCardProps> = ({ event, onTap, className = "" }) => {
  ...
  useEffect(() => {
    ...
    const attendeeId = `${event.id}_${user.uid}`;
  ...
  useEffect(() => {
    if (!event.id) return;
    return subscribeToEventAttendees(event.id, setAttendees);
  }, [event.id]);
